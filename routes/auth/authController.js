...
const authLoginController = (req, res, next) => {
  ...
  if (isValidUser) {
    res.status(200).cookie("isLoggedIn", "true").send("Login Succesful!"); //cookie sent on user login now we can read this cookie in other routes req object e.g lets use /dashboard route to read this cookie now AND we will send response on these routes like (secret) when we find cookie already in thier requset else not
  } else {
